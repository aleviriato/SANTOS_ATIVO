CREATE OR REPLACE PACKAGE PAC_CALCULA_FOLHA_ATIVOS is
  ...
  vi_ide_cli_adalim          varchar2(20):=null;
  v_data_ini_ref             date;
  PAR_TIPO_PROC_13           CHAR(1);
  v_num_ord_jud              NUMBER(8);
  --FIM #DECTEC2016  
...
--         'ATIVO'                            AS COM_TIP_BENEFICIO,
          ...
          (
             SELECT EV.COD_CARGO FROM TB_EVOLUCAO_FUNCIONAL EV
                   WHERE
                   EV.COD_INS          =  PAR_COD_INS         AND
                   EV.COD_IDE_CLI      =  ben_ide_cli         AND    
                   EV.NUM_MATRICULA    =  BB.NUM_MATRICULA    AND
                   EV.COD_IDE_REL_FUNC =  BB.COD_IDE_REL_FUNC AND
                  (to_char(PAR_PER_PRO, 'YYYYMM') >=
                   to_char( EV.DAT_INI_EFEITO, 'YYYYMM')      AND
                   to_char(PAR_PER_PRO, 'YYYYMM') <=
                   to_char(nvl(EV.DAT_FIM_EFEITO, to_date('01/01/2045', 'dd/mm/yyyy')),
                    'YYYYMM'))
                    AND NVL(EV.FLG_STATUS,'V')='V'
                    AND ROWNUM <2
...
/*           ,
  ...
  ---------------------------------------------------------------------------
  PROCEDURE SP_OBTEM_SOMA_VENCIMENTO(i_cod_entidade  in number,
...
/* PROCEDURE SP_COMPOSICAO_BENEFICIO(I_COD_RUBRICA   IN NUMBER,
 ...
 FUNCTION  SP_CALC_PORCENTUAL13 Return number;
                       
  --INICIO 13---------------------------------------------  
  PROCEDURE SP_GERA_DEBITO_AD_13;
  -------------------------------------------------------- 
  --PROCEDURE SP_AJUSTA_TETO13 ;
  --------------------------------------------------------
  PROCEDURE SP_OBTEM_VALOR_BASE_13  
                       (I_COD_INS          IN TB_PESSOA_FISICA.COD_INS%TYPE
                       ,I_COD_IDE_CLI      IN TB_PESSOA_FISICA.COD_IDE_CLI%TYPE
                       ,I_COD_ENTIDADE     IN TB_ENTIDADE.COD_ENTIDADE%TYPE
                       ,I_NUM_MATRICULA    IN TB_RELACAO_FUNCIONAL.NUM_MATRICULA%TYPE
                       ,I_COD_IDE_REL_FUNC IN TB_RELACAO_FUNCIONAL.COD_IDE_REL_FUNC%TYPE
                       ,O_VALOR           OUT NUMBER);
  --FIM 13------------------------------------------------                        
                       
                       
  -------------------------------------------------------- 
  --PROCEDURE SP_GET_DIAS_LIC_SEM_REMUN;
  --------------------------------------------------------
  PROCEDURE SP_GET_DIAS_LIC_SEM_REMUN  
                       (I_COD_INS          IN TB_PESSOA_FISICA.COD_INS%TYPE
                       ,I_COD_IDE_CLI      IN TB_PESSOA_FISICA.COD_IDE_CLI%TYPE
 ...
 end     PAC_CALCULA_FOLHA_ATIVOS;
/
CREATE OR REPLACE PACKAGE BODY PAC_CALCULA_FOLHA_ATIVOS is
  -------------------------------------------------------------------------------------
  PROCEDURE SP_FOLHA_CALCULADA3(i_cod_ins                in number,
...
--  nao_tem_prev           char(1)     := null;
  ...
  BEGIN
    BEGIN
      ...
      PAR_NUM_PROCESSO  := i_num_processo;
      PAR_NUM_GRP_PAG   := i_num_grp; 
      ...
      FOR J IN 1 .. num_cpf.count LOOP
        ...
        WHILE CURATIV%FOUND LOOP
          ...
          IF CUR_COMPATIV%found then
            ...
            --GERA RUBRICA DE DEC TER
            if (PAR_TIP_PRO = 'T') then
                 SP_GERA_DEBITO_AD_13; --#DECTEC2016 depende do SP_INCLUI_BENEFICIO_ARRAY;
            end if;
...
--          o_valor := round(o_valor * PAR_PERCENT_CORRECAO,2);
            v_sal_base(com_cod_beneficio)(1) := SP_OBTEM_SALARIO_BASE_CARGO(ben_cod_referencia); --#DECTER2016 TONYCARAVANA 2016-12-16
                     ...
                     ---- Bloque de IR ACUMULADO do Periodo ----
                             ...
                             ------- BLOQUE DE CALCULO DE IR DE 13 SALARIO ------
                              ...
                              IF V_VAL_IR > 0 OR V_VAL_IR_13 > 0 THEN
                                 ...
                                 SP_OBTEM_DETALHE_PAG(ANT_IDE_CLI, 'D', 'I', 'T');
                              ELSE
                                 NULL;
                              END IF;
...
/*                      DECLARE
                         V_RUBRICA NUMBER(8) := 0;
                      BEGIN
                          NULL;                      
                      END;
                    
                      --RECUPERANDO VALOR DA RUBRICA 13001
                       
                      V_VAL_IR := V_VAL_IR - V_RUBRICA; */
...
--    i_flg_retorno := 0;
  ...
  BEGIN
            /*UPDATE TB_FOLHA_PAGAMENTO_DET D 
               SET D.DES_INFORMACAO    = '30' --30 DIAS
             WHERE D.COD_FCRUBRICA    IN (100,101)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
               AND D.DES_INFORMACAO IS NULL;*/
             
            --INSS
            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET D.DES_INFORMACAO = (SELECT (CASE WHEN (D2.VAL_RUBRICA <= 1556.94) THEN
                                                   '8,00'
                                                   WHEN ((D2.VAL_RUBRICA >= 1556.95) AND (D2.VAL_RUBRICA <= 2594.92)) THEN
                                                   '9,00'
                                                   WHEN (D2.VAL_RUBRICA >= 2594.93) THEN
                                                   '11,00'
                                              END)
                                         FROM TB_FOLHA_PAGAMENTO_DET D2
                                        WHERE D2.COD_INS          = D.COD_INS
                                          AND D2.COD_ENTIDADE     = D.COD_ENTIDADE
                                          AND D2.COD_IDE_CLI      = D.COD_IDE_CLI
                                          AND D2.NUM_MATRICULA    = D.NUM_MATRICULA
                                          AND D2.COD_IDE_REL_FUNC = D.COD_IDE_REL_FUNC
                                          AND D2.PER_PROCESSO     = D.PER_PROCESSO
                                          AND D2.SEQ_PAGAMENTO    = D.SEQ_PAGAMENTO
                                          AND D2.COD_FCRUBRICA    = 99000)
             WHERE D.COD_FCRUBRICA IN (52800,52801)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.COD_IDE_CLI       = ANT_IDE_CLI
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
               AND D.DES_INFORMACAO IS NULL;
            ...
            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET D.DES_INFORMACAO = (SELECT (CASE WHEN (D2.VAL_RUBRICA <= 1556.94) THEN
                                        ...
                                        WHERE D2.COD_INS          = D.COD_INS
                                          ...
                                          AND D2.PER_PROCESSO     = D.PER_PROCESSO
                                          AND D2.SEQ_PAGAMENTO    = D.SEQ_PAGAMENTO
                                          AND D2.COD_FCRUBRICA    = 99700)
             WHERE D.COD_FCRUBRICA IN (35000,35001)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
            ...
            --IR
            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET D.DES_INFORMACAO = (SELECT (CASE WHEN (D2.VAL_RUBRICA <= 1903.98) THEN
                                                   '0,0'
                                                   WHEN ((D2.VAL_RUBRICA >= 1903.99) AND (D2.VAL_RUBRICA <= 2826.65)) THEN
                                                   '7,5'
                                                   WHEN ((D2.VAL_RUBRICA >= 2826.66) AND (D2.VAL_RUBRICA <= 3751.05)) THEN
                                                   '15,0'
                                                   WHEN ((D2.VAL_RUBRICA >= 3751.06) AND (D2.VAL_RUBRICA <= 4664.68)) THEN  
                                                   '22,5'                                                                              
                                                   WHEN (D2.VAL_RUBRICA  >= 4664.69) THEN
                                                   '27,5'
                                                   ELSE 
                                                   ''
                                              END)
                                        ...
                                        WHERE D2.COD_INS          = D.COD_INS
                                          ...
                                          AND D2.PER_PROCESSO     = D.PER_PROCESSO
                                          AND D2.SEQ_PAGAMENTO    = D.SEQ_PAGAMENTO
                                          AND D2.COD_FCRUBRICA    = 99200)
             WHERE D.COD_FCRUBRICA IN (50200)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
            ...
            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET D.DES_INFORMACAO = (SELECT (CASE WHEN (D2.VAL_RUBRICA <= 1903.98) THEN
                                        ...
                                        WHERE D2.COD_INS          = D.COD_INS
                                          ...
                                          AND D2.PER_PROCESSO     = D.PER_PROCESSO
                                          AND D2.SEQ_PAGAMENTO    = D.SEQ_PAGAMENTO
                                          AND D2.COD_FCRUBRICA    = 99900)
             WHERE D.COD_FCRUBRICA IN (35100,35101)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
            ...
            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET D.DES_INFORMACAO = NVL(SP_CALC_PORCENTUAL13,'12') || '/12' 
             WHERE D.COD_FCRUBRICA IN (30400,30401)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
            ...
            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET VAL_RUBRICA       = 5189.8200
                 , VAL_RUBRICA_CHEIO = 5189.8200
                 , NOM_PRO_ULT_ATU   = 'BASE_INSS'
             WHERE COD_FCRUBRICA     IN (99000,99700)
               AND VAL_RUBRICA      >= 5189.8200
               AND D.COD_INS         = PAR_COD_INS
               AND D.TIP_PROCESSO    = PAR_TIP_PRO
               AND D.PER_PROCESSO    = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO   = vi_seq_pagamento
...
/*            UPDATE TB_FOLHA_PAGAMENTO_DET D
               SET D.DES_INFORMACAO = (SELECT PE.VAL_UNIDADE
                                         FROM TB_COMPOSICAO_PAG_EVENTUAL PE
                                        WHERE PE.COD_INS       = D.COD_INS
                                          AND PE.COD_ENTIDADE  = D.COD_ENTIDADE
                                          AND PE.COD_IDE_CLI   = D.COD_IDE_CLI
                                          AND PE.NUM_MATRICULA = D.NUM_MATRICULA
                                          AND PE.COD_IDE_REL_FUNC = D.COD_IDE_REL_FUNC
                                          AND PE.COD_FCRUBRICA    = D.COD_FCRUBRICA
                                          AND PE.DAT_FIM_VIG IS NULL)
             WHERE D.COD_FCRUBRICA IN (400, 401, 200, 201, 300, 301)
               AND D.COD_INS           = PAR_COD_INS
               AND D.TIP_PROCESSO      = PAR_TIP_PRO
               AND D.PER_PROCESSO      = PAR_PER_PRO
               AND D.SEQ_PAGAMENTO     = vi_seq_pagamento
 ...
 PROCEDURE SP_PROCESSA_RUBRICA(CALC_RUB OUT BOOLEAN) AS
  ...
  BEGIN
    ...
    ELSE
      ...
      IF vi_condicao or vi_sem_condicao THEN
        ...
        IF (COM_TIPO_EVENTO_ESPECIAL = 'G' AND COM_TIP_BENEFICIO = 'APOSENTADO') THEN
          d := nvl(d, 0) + 1;
        ...
        IF (COM_TIP_BENEFICIO = 'APOSENTADO' AND COM_TIPO_EVENTO_ESPECIAL = 'D') THEN
           COM_FLG_PROCESSA := 'N';
        END IF;
  ...
  BEGIN
    ...
    ELSIF i_num_funcao = 31 THEN
      ...
      END IF;
      -- o_valor := round(o_valor * PAR_PERCENT_CORRECAO , 2);
      -- v_sal_base(com_cod_beneficio)(1) := o_valor; --Comentado por Tony Caravana Campos em 2016-09-21, pois a func?o 6 ja tras o salario base
    ...
    ELSIF i_num_FUNCAO = 44 THEN
      ...
      SP_OBTEM_SOMA_VENCIMENTO(com_cod_entidade,
                               'N',
                               o_valor);
...
/*      SP_COMPOSICAO_BENEFICIO
    ...
    ELSIF  i_num_FUNCAO = 531 THEN
         ...
         o_valor:= SP_CALC_PORCENTUAL13;
         
         

   ------ NOVO SET DE FUNC?ES CRIADAS POR TONY CARAVANA CAMPOS #TONYCARAVANA2016
   
   ELSIF  i_num_FUNCAO = 532 THEN
   
       -- Valor deducivel por dependente economico
       SP_OBTEM_PARVAL_FOLHA2('IMPDE', 1000, 'IMPDE', o_valor);

   ELSIF I_NUM_FUNCAO = 533 THEN
   
        SELECT SUM(D.VAL_RUBRICA) 
          INTO o_valor
          FROM TB_HFOLHA_PAGAMENTO_DET D 
         WHERE D.COD_INS = PAR_COD_INS
           AND D.TIP_PROCESSO  = 'N'
           AND D.PER_PROCESSO  = ADD_MONTHS(PAR_PER_PRO,-1)
           AND D.COD_IDE_CLI   = COM_COD_IDE_CLI
           AND D.SEQ_PAGAMENTO = 1
           AND D.COD_FCRUBRICA IN (
                    SELECT D.COD_FCRUBRICA_COMPOE
                      FROM TB_COMPOE_DET D
                     WHERE D.COD_FCRUBRICA_COMPOSTA = '9000'
               );
   ...
   ELSIF I_NUM_FUNCAO = 534 THEN
       ...
       DECLARE 
           v_contaa     VARCHAR2(256);
           v_rdcn_temp  TB_DET_CALCULADO_ATIV_ESTRUC%rowtype;
       BEGIN
          v_contaa := tdcn.count;
          IF v_contaa > 0 then
            FOR X IN 1 .. v_contaa LOOP
              v_rdcn_temp := tdcn(X);
              IF v_rdcn_temp.COD_FCRUBRICA = 12801 THEN
--               AND rdcn_temp.cod_beneficio = codbenef THEN
                 o_valor := v_rdcn_temp.val_RUBRICA;
              END IF;
            END LOOP;
          END IF;
       END;
       ...
       ELSIF I_NUM_FUNCAO = 559 THEN
       
            SP_GET_DIAS_LIC_SEM_REMUN
                      (PAR_COD_INS
                      ,COM_COD_IDE_CLI
                      ,COM_ENTIDADE
                      ,COM_NUM_MATRICULA
   ...
   ELSIF  i_num_FUNCAO = 10002 THEN
             BEGIN
               select T.COD_PODER_APLIC 
                 INTO o_str
                 FROM TB_REGRA_APLIC_TETO_CARGO T
                WHERE T.cod_ins         = PAR_COD_INS
                  AND T.COD_ESTRUTURA   = 1000
                  AND T.COD_CARGO_APLIC = COM_CARGO
                  AND ROWNUM <2;
             EXCEPTION
               when no_data_found then
                 o_str:='TETO_PREF';
   ...
   END IF;
    ...
    IF i_num_funcao in
       ( 99,   3, 4, 8, 16, 19, 25, 15, 29, 48, 51, 33, 34, 35, 36, 37, 38, 39, 47, 67, 70, 72, 78, 94, 101, 105, 108, 109, 113, 114, 118, 135,136, 201, 210, 211, 212,403,404,411,412,414,523,528 ) THEN
      return(o_str);
    ELSE
    ...
    BEGIN
      ...
      select distinct  
        ...
        where rf.cod_ins         = PAR_COD_INS
         and rf.cod_ide_cli      = COM_COD_IDE_CLI --ADICIONADO POR TONY CARAVANA CAMPOS #OPERACAOACERTOS2016
...
/*          SP_INCLUI_DETALHE_PAG (v_ant_cod_benef,
  ...
  begin
    ...
    BEGIN
      COD_BENEFICIO_RET:=0;
      FOR  INDEX_BEN   IN 1 .. vcount LOOP
...
/*                  SELECT RET.cod_ins,
                    ...
                    FROM tb_det_ret RET, TB_CONCESSAO_BENEFICIO CC,
                         TB_RUBRICAS RU
                   WHERE RET.cod_ins = PAR_COD_INS
                     ...
                     AND per_processo = PAR_PER_PRO
                     AND (dat_ini_ref <PAR_PER_PRO or (dat_ini_ref <=PAR_PER_PRO and PAR_TIP_PRO='S' ))
                  ...
                  SELECT RET.cod_ins,
                         ...
                         CC.COD_ENTIDADE,
                         RU.TIP_EVENTO_ESPECIAL
                    FROM tb_valor_npago_ret RET, TB_CONCESSAO_BENEFICIO CC,
                         TB_RUBRICAS RU
                   WHERE RET.cod_ins = PAR_COD_INS
                     AND cod_ide_cli = ANT_IDE_CLI
                     AND CC.COD_BENEFICIO = COD_BENEFICIO_RET
...
--                rdcn.cod_beneficio := CRET_COD_BENEFICIO;
  ...
  PROCEDURE SP_CALCULA_IMPOSTO2(TotBru in number, MonImp out number) AS
    ...
    rdcn_temp TB_DET_CALCULADO_ATIV_ESTRUC%rowtype;
  ...
  BEGIN
    ...
    BEGIN
      ...
      --INICIO
      IF tdcn.count > 0 then
        FOR X IN 1 .. tdcn.count LOOP
            rdcn_temp := tdcn(X);
            IF rdcn_temp.COD_FCRUBRICA = 13001 THEN

                MonImp := MonImp - rdcn_temp.val_rubrica;
 ...
 /*               COM_COD_RUBRICA            := vi_rubrica;
                ...
                COM_TIPO_EVENTO_ESPECIAL     := I_TIP_EVENTO_ESPECIAL;
                ...
                ben_ide_cli := ben_ide_cli_val_orig;*/
...
--        #DECTER2016 - TONY CARAVANA 2016-12-17 (Alterar tip_evento_especial = 3 para tip_evento = 'T'
          IF PAR_TIP_PRO = 'T' AND TRIM(NVL(RDCN.TIP_EVENTO_ESPECIAL,' ')) = '3' THEN
...
-------------------------------------------
 ...
 FUNCTION  SP_CALC_PORCENTUAL13 Return number IS
...
--         rfol := vfolha(i);
            ...
            ---------- OBTEM ADIANTAMENTO 13 PAGO NO ANO.---------
               ...
               BEGIN
                    SELECT
                     MIN(DET.DAT_INI_REF),
                     MIN(decode(TRUNC(DET.COD_FCRUBRICA / 100),90,11000,DET.COD_FCRUBRICA)),
                     NVL(sum(
                     ...
                     WHERE DET.COD_INS          = 1
                       ...
                       AND DET.PER_PROCESSO >= DAT_INICIO_PERIODO13
                       AND DET.PER_PROCESSO <  DAT_TERMIN_PERIODO13
                                              
                       AND DET.DAT_INI_REF     >= DAT_INICIO_PERIODO13
                       AND DET.DAT_INI_REF     <= DAT_TERMIN_PERIODO13
                       
                       AND DET.COD_IDE_CLI      = COM_COD_IDE_CLI
                       AND DET.COD_ENTIDADE     = COM_ENTIDADE                       
                       AND DET.NUM_MATRICULA    = COM_NUM_MATRICULA
                       AND DET.COD_IDE_REL_FUNC = COM_COD_IDE_REL_FUNC
                       
                       AND EXISTS  (SELECT 1
                                      FROM  TB_RUBRICAS RR
                                     WHERE RR.COD_INS      = PAR_COD_INS
                                       AND RR.COD_CONCEITO = TRUNC(DET.COD_FCRUBRICA / 100)
                                       AND RR.TIP_EVENTO_ESPECIAL IN ('D', 'G')
                                       AND RR.COD_ENTIDADE = COM_ENTIDADE
                                    )
/*                      AND   DET.COD_FCRUBRICA NOT IN (1860750,1860756,1860702)
                      and not (det.cod_fcrubrica in (2300050,2200050,2200051,2300051,1860750,1860751,2300750,2300755,2300756,2302056) and det.dat_ini_ref=DAT_INICIO_PERIODO13)*/
                      ;
          ...
          IF vi_val_rubrica > 0 THEN
             ...
             SP_INCLUI_DETALHE_PAG(COM_NUM_MATRICULA,
                                   COM_COD_IDE_REL_FUNC,
                                   COM_ENTIDADE,
                                   vi_rubrica,
                                   vi_val_rubrica,
...
--        rfol := vfolha(i);
          ...
          END IF;
         -------------------- VALOR ADIANTAMENTO TETINHO ---------------
             V_DATA_INI_REF:=NULL;
             BEGIN
                   SELECT
                   MIN(DET.DAT_INI_REF),
                   MIN( DET.COD_FCRUBRICA) ,
                   ...
                   WHERE DET.COD_INS = PAR_COD_INS
                     ...
                     AND DET.DAT_INI_REF  >= DAT_INICIO_PERIODO13
                      ...
                      -- Comentario abaixo mudado o 22-06-2012
                      -- para considerar os adiantamento anteriores
                      -- a dec do ano que correspodem ao novo periodo
                    -- AND   DET.DT_FECHAMENTO <=DAT_TERMIN_PERIODO13
                     AND DET.DAT_INI_REF    <=DAT_TERMIN_PERIODO13
                   ...
                   (SELECT 1
                           ...
                           WHERE RR.COD_INS = 1
                             ...
                             AND RR.TIP_EVENTO_ESPECIAL IN ('U')
                             AND RR.COD_ENTIDADE = COM_ENTIDADE)
                             AND TRUNC(DET.COD_FCRUBRICA/100)=21604;
          exception
            when no_data_found then
                        vi_rubrica:=NULL;
                        vi_val_rubrica :=0;
          ...
          END;
            ...
            IF vi_val_rubrica < 0 THEN
               vi_val_rubrica := vi_val_rubrica *-1;
               IF par_tipo_proc_13 ='R'  THEN
                 vi_rubrica:=2160400;
               ELSE
                 vi_rubrica:=2160402;
               END IF;  
            ...
            END IF;
            ----------------  CONTRIBUIC?O DE ADIANTAMENTO 13 -----------------
             V_DATA_INI_REF:=NULL;
             BEGIN
                   SELECT
                   MIN(DET.DAT_INI_REF),
...
--                   AND DET.DT_FECHAMENTO>=DAT_INICIO_PERIODO13
                   ...
                   (SELECT 1
                           ...
                           WHERE RR.COD_INS = 1
                             AND RR.COD_CONCEITO = TRUNC(DET.COD_FCRUBRICA / 100)
                             AND RR.TIP_EVENTO_ESPECIAL IN ('F')
                             AND RR.COD_ENTIDADE = COM_ENTIDADE)
                             AND TRUNC(DET.COD_FCRUBRICA/100)=70062;
          exception
            when no_data_found then
                        vi_rubrica:=NULL;
                        vi_val_rubrica :=0;
          ...
          END IF;
            ...
            ------------ LANCAMENTO DE PA DE ADIANTAMENTO-----------------------
            ...
            vi_ide_cli_adalim :=null;
                      ...
                      FOR ALIM IN (
                             SELECT
                              MIN(DET.DAT_INI_REF) dat_referencia,
                              to_char(MIN(SUBSTR(DET.COD_FCRUBRICA, LENGTH(DET.COD_FCRUBRICA)-1,2))) cod_final ,
                              NVL( sum(
                               ...
                               DET.COD_IDE_CLI_BEN cod_ide_alimentado
                               FROM TB_HFOLHA_PAGAMENTO_DET DET
                             WHERE DET.COD_INS = PAR_COD_INS
                               AND DET.TIP_PROCESSO IN ('N', 'S')                           
...
--                             AND DET.DT_FECHAMENTO>=DAT_INICIO_PERIODO13
                             ...
                             (SELECT 1
                                     ...
                                     WHERE RR.COD_INS = PAR_COD_INS
                                       AND RR.COD_CONCEITO =
                                           TRUNC(DET.COD_FCRUBRICA / 100)
                                       AND RR.COD_ENTIDADE = COM_ENTIDADE)
                                       AND TRUNC(DET.COD_FCRUBRICA/100) IN (78016/*, 78028*/)
                                       AND
                                       EXISTS (
                                        select 1 from TB_COMPOSICAO_PAG_EVENTUAL v
                                        where v.cod_ins=PAR_COD_INS 
                                          and v.cod_ide_cli      = det.cod_ide_cli  
                                          AND v.cod_entidade     = det.cod_entidade
                                          AND v.num_matricula    = det.num_matricula
                                          AND v.cod_ide_rel_func = det.cod_ide_rel_func 
                                          AND v.cod_ide_cli_ben=det.cod_ide_cli_ben
                                          
                                          AND v.dat_fim_vig is null          
                                          AND v.flg_status='V'               
                                          AND exists (
                                              select 1 
                                                from tb_rubricas rr
                                               where rr.cod_ins     = PAR_COD_INS 
                                                 and rr.cod_rubrica = v.cod_fcrubrica 
                                                 and rr.tip_evento_especial='P'                                          
                                               )
                                         )                                      
                      group by DET.COD_IDE_CLI_BEN
                     )
                LOOP
                        vi_ide_cli_adalim :=alim.cod_ide_alimentado;
                      IF ALIM.valor_pa < 0 THEN
                         vi_val_rubrica := ALIM.valor_pa *-1;
                         vi_rubrica:=TO_NUMBER('78022'||alim.cod_final);
                         V_DATA_INI_REF:=alim.dat_referencia;
             ...
             ------------------------------------------------
                          FOR I IN 1 .. TDCN.COUNT LOOP
                                RDCN := NULL;
                                RDCN := TDCN(I);
                                IF   trunc(RDCN.cod_fcrubrica / 100, 000) IN (
                                  78001	, 78022	, 78036	, 78006	, 78020	,78038	,
                                  79017	, 79021	, 78018	, 78009	, 78010	,79018	,
                                  78037	, 78007	, 79019	, 78016	, 79020	,78017	,                          78034	,
                                  78008	, 79031	, 78019	, 78028	, 78030	,78033	,78035	
               
                                ) and
                                    
                                    rfol.cod_ide_cli      = RDCN.cod_ide_cli and
                                    rfol.COD_ENTIDADE     = RDCN.cod_entidade and
                                    rfol.NUM_MATRICULA    = RDCN.num_matricula and
                                    rfol.COD_IDE_REL_FUNC = RDCN.COD_IDE_REL_FUNC and
                                    
                                    vi_ide_cli_adalim  = RDCN.cod_ide_cli_ben THEN
                                    EXISTE_PA:=1;
                                  exit;  
                                END IF;
                           END LOOP;
...
--       END LOOP;
     ...
     ADIANTAMENTO:=FALSE; --VERIFICAR

END SP_GERA_DEBITO_AD_13;

/*PROCEDURE SP_AJUSTA_TETO13 AS
  cod_benef  NUMBER;
  vi_rubrica NUMBER;
  vi_seq_vig NUMBER;
  I          NUMBER;
  I2         NUMBER;
  PORCETUAL_13 number (10,6);
BEGIN
     ...
     FOR i IN 1 .. v_cod_beneficio.count LOOP
        ...
        SP_OBTEM_RUBRICA_EVENTO_ESPEC('E',
         ...
         FOR ITDN IN 1 .. tdcn.count LOOP
            IF tdcn(ITDN).COD_BENEFICIO =RFOL.COD_BENEFICIO AND
               tdcn(ITDN).COD_FCRUBRICA =vi_rubrica THEN
                IF (PAR_TIP_PRO = 'T' ) THEN
                         PORCETUAL_13:=SP_VALOR_PORCENTUAL13(cod_benef);
                        tdcn(ITDN).val_rubrica       :=round(tdcn(ITDN).val_rubrica  * PORCETUAL_13,2);
                        tdcn(ITDN).val_rubrica_cheio :=round(tdcn(ITDN).val_rubrica_cheio *PORCETUAL_13,2);
              END IF;
            END IF;
...
BEGIN
     ...
     SELECT SUM(VAL_RUBRICA)
       INTO O_VALOR
       FROM TB_FOLHA_PAGAMENTO_DET DC
          , TB_FOLHA_PAGAMENTO     FO
      WHERE DC.PER_PROCESSO     = PAR_PER_PRO
        AND DC.TIP_PROCESSO     = 'N'
        AND DC.COD_INS          = I_COD_INS
        AND DC.COD_IDE_CLI      = I_COD_IDE_CLI
        AND DC.COD_ENTIDADE     = I_COD_ENTIDADE
        AND DC.NUM_MATRICULA    = I_NUM_MATRICULA
        AND DC.COD_IDE_REL_FUNC = I_COD_IDE_REL_FUNC
        ...
        AND DC.COD_FCRUBRICA IN (
            SELECT CP.COD_FCRUBRICA_COMPOE 
              FROM TB_COMPOE_DET CP
             WHERE CP.COD_INS                = FO.COD_INS
               AND CP.COD_ENTIDADE_COMPOSTA  = FO.COD_ENTIDADE
               AND CP.COD_FCRUBRICA_COMPOSTA = COM_COD_FCRUBRICA
        );
        
      EXCEPTION 
          WHEN OTHERS THEN 
               O_VALOR := 0; 
END SP_OBTEM_VALOR_BASE_13;
...
PROCEDURE SP_GET_DIAS_LIC_SEM_REMUN  
      ...
      BEGIN                       
          ...
          SELECT COUNT(*) AS QTD_DIAS_LIC_DESC_FOLHA
            INTO O_VALOR
            FROM TB_AFASTAMENTO        A
               , TB_MOTIVO_AFASTAMENTO B
               , TB_PARAM_MOTIVO_AFAST C
           WHERE COD_INS          = I_COD_INS
             AND COD_IDE_CLI      = I_COD_IDE_CLI
             AND COD_ENTIDADE     = I_COD_ENTIDADE
             AND NUM_MATRICULA    = I_NUM_MATRICULA
             AND COD_IDE_REL_FUNC = I_COD_IDE_REL_FUNC
             AND A.COD_MOT_AFAST  = B.COD_MOT_AFAST
             AND C.COD_MOT_AFAST  = B.COD_MOT_AFAST
             AND C.FLG_DESCONTA_FOLHA = 'S';
                       
      EXCEPTION 
          WHEN OTHERS THEN 
               O_VALOR := 0; 
END SP_GET_DIAS_LIC_SEM_REMUN;
